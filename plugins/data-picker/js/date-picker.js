...
var selectDate;
var urls="js/data.json";
//获取不可选日期
AjaxDate(urls,"",function(data){
    console.log(data);
    unavailableDatePool=data;
});
...
var plugin = {
    ...
    initPlugin: function (ele, args, fn) {
        ...
        var handles = {
            ...
            config:args,
            time_show:args.time_show||false
        };
        this.count++;
        if (!this.isCssLoaded) {
        ...
        var preHtml =
            "<div style='display: inline-block; padding: 0 10px 10px 10px; border: 1px solid #777777;min-width: 200px'>" +
            "<table>" +
            ...
            //"<label>无法预定</label><div class='dp-used'></div>" +
            //"<label>预定</label><div class='dp-selected'></div>" +
        ...
        if (handles.withInput === "true") {
            preHtml +=
                ...
                "<label>开始时间<input placeholder='请点击日历' class='js-begin-date' readonly/></label>" +
                "<label>结束时间<input class='js-end-date' placeholder='请点击日历' readonly/></label>" +
                "</td>" +
                "</tr>" +
                "</table>" +
                "</div>";
...
function checkIfValid(inputTimestamp) {
    if (unavailableDatePool!=undefined&&unavailableDatePool.length <= 0) {
        return true;
    } else {
...
function containsValid(left, right) {
    if(unavailableDatePool!=undefined){
        for (var i = 0; i < unavailableDatePool.length; i++) {
...
function displayCurrentMonth(year, currentMonth) {
    ...
    domHandle.find(".dp-date").unbind("click").bind("click", function (e) {
        var self = this;
        if(handles.time_show==true){
            e.stopPropagation();
            if ($(self).hasClass("disabled")) {
                return;
            }
            $("td[data-date='all-day']").trigger("click",function(){
            ...
            showDetailDate(self, function (dateContent) {
                if (refreshDatePool.call(handles, self, dateContent)) {
                    ...
                    updateDateInput.call(handles, datePool[0]);
                    handles.datePool = datePool;
                    if($(handles.config.input).length>0){
                        var newDate=handles.datePool.pop();
                        var t = dateToString(newDate);
                        $(handles.config.input).val(t);
                    }
                }
            });
            return;
            ...
            if (refreshDatePool.call(handles, self, "")) {
                ...
                if($(handles.config.input).length>0){
                    var newDate=handles.datePool.pop();
                    var t = dateToString(newDate);
                    var da1=$(handles.config.input).first().val();
                    var da2=$(handles.config.input).eq(1).val();
                    if(da1!=""&&da2!=""){

                        //$(handles.config.input).val("");
                    }
                    if($(handles.config.input).eq(1).val()!=""){
                        $(handles.config.input).val("");
                    }
                    if($(handles.config.input).val()==""){
                        $(handles.config.input).first().val(t);
                    }else{
                        if( $(handles.config.input).first().val()!=""){
                            d2=t;
                            var date1= $(handles.config.input).first().val();
                            if(d2<date1){
                                $(handles.config.input).first().val(t);
                            }else{
                                $(handles.config.input).eq(1).val(t);
                            }
                            var dat1= $(handles.config.input).eq(0).val();
                            var dat2= $(handles.config.input).eq(1).val();
                            var dateString1 = Date.parse(new Date(dat1));
                            var dateString2 = Date.parse(new Date(dat2));
                            if(dateString1>dateString2){
                                $(handles.config.input).eq(0).val(dat2);
                                $(handles.config.input).eq(1).val(dat1);
                            }
                        }
                    }

                    //增加遮罩选取日期
                    if($(handles.config.input).first().val()!=""&&$(handles.config.input).eq(1).val()!=""){
                        if($(".cover1").length>0){
                            $(".date,.cover1").fadeOut();
                        }
                    }
...
function showDetailDate(obj, fn) {
    ...
    virtualHtml +=
        "'><table>" +
        "<tr><td>0:00</td><td>1:00</td><td>2:00</td><td>3:00</td></tr>" +
        "<tr><td>4:00</td><td>5:00</td><td>6:00</td><td>7:00</td></tr>" +
        "<tr><td>8:00</td><td>9:00</td><td>10:00</td><td>11:00</td></tr>" +
        "<tr><td>12:00</td><td>13:00</td><td>14:00</td><td>15:00</td></tr>" +
        "<tr><td>16:00</td><td>17:00</td><td>18:00</td><td>19:00</td></tr>" +
        "<tr><td>20:00</td><td>21:00</td><td>22:00</td><td>23:00</td></tr>" +
        //"<tr><td colspan='2' data-date='all-day'>全天</td>" +
        //"<td colspan='2' data-date='cancel-date'>取消</td>" +
        "</tr>" +
        "</table>" +
        "</div>";
...
function dateToString(time){
    var test = new Date(parseInt(time));
